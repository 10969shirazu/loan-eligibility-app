...
# # preprocessing_script.py avec label sauvegardé ce qu'on n'a pas fait dans le notebook
...
# # Fonction de prétraitement complet pour tester une nouvelle donnée
# def preprocess_input(test_data: pd.DataFrame) -> pd.DataFrame:
#     # Appliquer les LabelEncoders sauvegardés
#     cat_cols = ['term', 'emp_length', 'home_ownership', 'verification_status', 'purpose', 'grade', 'sub_grade']

#     for col in cat_cols:
#         if col in test_data.columns and col in label_encoders:
#             test_data[col] = label_encoders[col].transform(test_data[col].astype(str))
#         else:
#             test_data[col] = 0  # Valeur par défaut si la colonne n'est pas présente

#     # Ajouter les colonnes manquantes attendues par le modèle avec valeur par défaut 0
#     expected_features = [
#         'loan_amnt', 'term', 'int_rate', 'installment', 'grade', 'sub_grade',
#         'emp_length', 'home_ownership', 'annual_inc', 'verification_status',
#         'purpose', 'dti', 'delinq_2yrs', 'inq_last_6mths', 'open_acc',
...
#     # S'assurer de l'ordre des colonnes correct
